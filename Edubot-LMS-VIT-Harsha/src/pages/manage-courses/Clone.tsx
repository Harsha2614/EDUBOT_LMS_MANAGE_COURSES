...
const CloneCoursePage: React.FC = () => {
  const router = useRouter();

  const [courseName, setCourseName] = useState('Programming fundamentals');
  const [courseId, setCourseId] = useState('583RT');
  const [description, setDescription] = useState(
    'Programming fundamentals are the essential principles and concepts that form the foundation of writing and understanding code in any programming language.'
  );
  const [skillsGain, setSkillsGain] = useState(
    'Problem-solving, logical thinking, debugging, code optimization, understanding algorithms, and proficiency in various programming languages and tools.'
  );
  ...
  const handleSubmit = () => {
    ...
    console.log({ courseName, courseId, description, skillsGain, duration, isActive, modules });
  };
      ...
      <div className="tw-bg-white tw-rounded-md tw-shadow-md tw-p-4 md:tw-p-6 tw-space-y-4">
        ...
        <div>
          <label className="tw-font-medium">
            Course Name <span className="tw-text-gray-400">(Cloning requires new name)</span>
          </label>
          <input
            value={courseName}
        ...
        <div>
          <label className="tw-font-medium">
            Create Course ID <span className="tw-text-gray-400">(Cloning requires new ID)</span>
          </label>
          <input
            value={courseId}
        ...
        <div>
          <label className="tw-font-medium">
            Course Overview <span className="tw-text-gray-400">(Optional)</span>
          </label>
          <textarea
            value={description}
    ...
    <option value="8 weeks">8 weeks</option>
        ...
        <div className="tw-flex tw-flex-col md:tw-flex-row tw-gap-5">
            ...
            <ul className="tw-list-none tw-mt-2">
              {modules.map((mod, idx) => (
                <li
                  ...
                  className={`tw-p-2 tw-mb-1 tw-border-b tw-border-gray-300 tw-flex tw-justify-between tw-items-center tw-cursor-pointer ${
                    selectedModuleIndex === idx ? 'tw-bg-blue-100' : ''
                  }`}
                >
                  <span className="tw-break-words tw-max-w-[75%] sm:tw-max-w-[85%]">
                    {idx + 1}. {mod}
                  </span>
          ...
          <div className="tw-grid tw-grid-cols-2 tw-gap-4 tw-w-full max-md:tw-mt-3 md:tw-flex md:tw-flex-col md:tw-ml-4 md:tw-mt-6">
            {[{ label: 'Top', action: 'top' }, { label: 'Up', action: 'up' }, { label: 'Down', action: 'down' }, { label: 'Bottom', action: 'bottom' }].map(
              ({ label, action }) => {
                const Icon =
                  label === 'Top' ? MoveUp : label === 'Bottom' ? MoveDown : label === 'Up' ? ArrowUp : ArrowDown;
        ...
        <p className="tw-text-xs tw-text-gray-500 mt-4">*Cloning of program requires to change the program name.</p>
        <button
          onClick={handleSubmit}
          className="tw-bg-[#1A177A] tw-text-white tw-py-2 tw-px-6 tw-rounded tw-font-bold tw-w-full md:tw-w-[160px] tw-mt-2"
        >
          Clone Course
        </button>
      </div>
...
export default CloneCoursePage;
